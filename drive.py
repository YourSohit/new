...
class GoogleDriveHelper:
    ...
    def __init__(self, DRIVE_ID, name=None, path=None, bot_loop=None, message=None):
        self.__OAUTH_SCOPE = ['https://www.googleapis.com/auth/drive']
        self.__G_DRIVE_DIR_MIME_TYPE = "application/vnd.google-apps.folder"
        self.__G_DRIVE_BASE_DOWNLOAD_URL = "https://drive.google.com/uc?id={}&export=download"
        self.__G_DRIVE_DIR_BASE_DOWNLOAD_URL = "https://drive.google.com/drive/folders/{}"
        self.__path = path
        self.GDRIVE_FOLDER_ID = DRIVE_ID
        ...
        try:
            if ospath.isfile(item_path):
                ...
                link = self.__upload_file(
                    item_path, file_name, mime_type, self.GDRIVE_FOLDER_ID, is_dir=False)
                ...
                if len(getListOfFiles(item_path)) == 0:
                    LOGGER.info("Skipping upload of {ospath.abspath(file_name)} bcz its empty")
                    return "skip"
                dir_id = self.__create_directory(ospath.basename(
                    ospath.abspath(file_name)), self.GDRIVE_FOLDER_ID)
    ...
    def __create_directory(self, directory_name, dest_id):
        #is_exist = self.get_exist_folder_id(directory_name, dest_id)
        #if is_exist:
        #    return is_exist
        file_metadata = {
            "name": directory_name,
